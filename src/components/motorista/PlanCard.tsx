  ...
  return (
    <div
      className={`relative flex flex-col rounded-2xl border bg-white p-6 shadow-sm transition ${
        selecionado ? "border-brand-orange ring-2 ring-brand-orange/25" : "border-neutral-200"
      }`}
    >
      {plano.destaque && (
      ...
      <h3 className="text-lg font-semibold text-brand-navy">{plano.label}</h3>
      <p className="mt-1 text-3xl font-bold text-brand-navy">
        {formatarBRL(plano.valorBase)}
        <span className="text-sm font-normal text-neutral-500"> pré-pago</span>
      </p>
      <p className="text-sm text-neutral-500">{plano.cicloLabel}</p>
